...
| `v1-probes.sh <dir>` | The `rc` row's extra command in `v1-qualification.tsv`, run in the pinned tree: the December 24 family and #151's reported-speech sentences through `Tools/PipelineProbe` (#151's held set judged at the tree, its unchanged set at #151's own merge against that merge's first parent), #136's two DEL-11 scorers, #136's R1 timing against two fixed earlier commits, and the typed inputs for the handoff's M3. Inputs in `v1-probes/`; one PASS or FAIL line per step in `<dir>/steps.txt` | no |
