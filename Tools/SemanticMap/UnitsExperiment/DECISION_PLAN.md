...
**Remedy, pre-registered so it is not chosen after the answers exist (grader round 9, R4):** if the run is invalid only because a generation error's name is not on the list:
1. Check that name against Apple's documentation without reading any answer.
2. If it is the model's own behaviour, such as a refusal or a guardrail, add it to the list in its own commit and decide again on the same `results.jsonl`.
3. If it is a genuine infrastructure failure, and only then, regenerate.
...
| Guarded families | zero content cuts (an over-split, absorbed or wrong-boundaries flag) in reported speech, message content, or deliberation that ends in a decision. A unit that is only filler separates no content, so it spends the silent budget but does not fail this gate (grader round 9, R1) | A cut there separates content from its carrier or a decision from its negation. That is the exit gate's P0 shape |
...
- **Both qualify:**
  ...
  - Otherwise the decision line reads **NO CLEAR WINNER**, the last line reads `OUTCOME A (no clear winner)`, and the outcome is A by Calvin's stated conditions. Any report of that result must say "A without a clear margin". Candidate 1's condition holds: it materially improves on the baseline and its remaining failures are within the safety budgets. Candidate 2's does not: it did not materially outperform candidate 1, and "simplifies" is not measured by this experiment.
