...
final class CapturedItem: Identifiable {
    ...
    /// confirms a time.
    ///
    /// `ItemPresentation.mayArmTime` refuses these, so `scheduledDelivery` is
    /// `.none` and `ReminderScheduleRequest` makes no request. The capture-time
    /// hold already gives them no reminder date, so this is the second layer,
    /// for rows that carry one anyway: rows stored before the hold existed, and
    /// any producer outside `ThoughtOrganizer.organize`.
