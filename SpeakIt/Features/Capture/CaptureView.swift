...
struct CaptureView: View {
    ...
    private func endAttemptWithoutWords() {
        ...
        transcriber.cancel()
        discardActiveDraft()
        // Words typed before speaking are still in the editor, and this
        // attempt added nothing to them. The draft just discarded was their
        // only durable copy, so they are checkpointed again as a typed draft
        // of their own, with no recording, the way typing checkpoints them
        // (#144 with #139: the attempt ends, the typed words stay).
        if !typedBeforeSpeakingOnScreen.isEmpty {
