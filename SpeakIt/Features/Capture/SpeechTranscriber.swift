...
final class SpeechTranscriber: ObservableObject {
    ...
    /// The run that took the claim `holdsMicrophone` records, so a stale run
    /// can give back its own claim without releasing a newer run's. `nil`
    /// when nothing holds one, or when a test claimed with no run open.
    private var microphoneClaimStartID: UUID?
    ...
    func claimMicrophone() {
        ...
        microphoneClaimStartID = activeStartID
    }
    ...
    func start(
        ...
        do {
            ...
            // Nothing between the wait's own ownership check and here
            // suspends, so this run still owns the capture and the guard
            // cannot fail today. Not resetting is right only for an await
            // added between `audioEngine.start()` and this line: the stale run
            // has finished its writes, the start that superseded it already
            // reset them, and resetting now would tear down that newer run's
            // microphone (LIF-7), the way the stale branch in
            // `adoptStartedBackend` used to. It does owe the announcer the
            // claim `claimMicrophone()` took above, if that claim is still
            // this run's: returning with it would withhold every VoiceOver
            // announcement for the rest of the process, and releasing a newer
            // run's claim would let VoiceOver speak into its open microphone.
            // An await added anywhere earlier needs its own ownership check
...
enum CaptureAudioRecovery {
    ...
    static func transcribeRecording(of draft: CaptureDraftStore.Draft) async throws -> String {
        try await transcribeRecordingReportingEnding(of: draft).text
    }

    /// `transcribeRecording(of:)`, also saying which branch finished it
    /// (#148). It goes through `transcribeRecording(of:reading:)`, so an
    /// incomplete pass still keeps its words on the draft.
    static func transcribeRecordingReportingEnding(
        of draft: CaptureDraftStore.Draft
    ) async throws -> CaptureAudioRecoveryTranscript {
        var ending = CaptureAudioRecoveryEnding.recognizerFinal
        let text = try await transcribeRecording(of: draft, reading: { url in
            let recovered = try await CaptureAudioRecovery.transcribeAudioReportingEnding(at: url)
            ending = recovered.ending
            return recovered.text
        })
    ...
    /// `outcome(latest:ending:)` as the recovery reports it (#148). Only a
    ...
    static func reportedOutcome(
        latest: String?,
        ending: Ending
    ) -> Result<CaptureAudioRecoveryTranscript, Error> {
        outcome(latest: latest, ending: ending).map {
            CaptureAudioRecoveryTranscript(text: $0, ending: .recognizerFinal)
        }
    }
            ...
            try await withCheckedThrowingContinuation { continuation in
                ...
                let task = recognizer.recognitionTask(with: request) { result, error in
                    if let result {
                        ...
                        if result.isFinal {
                            completion.finish(with: CaptureAudioRecovery.reportedOutcome(
                                latest: text,
