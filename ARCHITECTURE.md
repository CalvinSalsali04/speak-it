...
Schema versioning keeps two things apart on purpose. Every numbered version in `SchemaV1.swift` — `SpeakItSchemaV1`, `SpeakItSchemaV2`, `SpeakItSchemaV3` — is history: each nests its own frozen `@Model` copies, none may ever be edited, and together they are what `SpeakItMigrationPlan` migrates a store *from*. `SpeakItSchemaCurrent` is the live shape the app addresses, is the only declaration allowed to move, and is what `PersistenceController.schema` opens. Collapsing the two — letting the newest numbered version name the live classes — is what makes adding the next version redefine the previous one and stop existing stores from opening. `SchemaFreezeTests` pins the version 3 contract to a real store a shipped build wrote, and fails the moment a persisted model changes shape without a new version.
