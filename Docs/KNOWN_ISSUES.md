...
*Appearance is applied by hand, not by `preferredColorScheme` (2026-09-10, narrowed 2026-09-11).* SwiftUI keeps the last non-nil value `preferredColorScheme` was given and re-asserts it whenever the traits change, so after the app had been in Light (the first-install default) choosing System left a light window that ignored the iPhone's own switch until relaunch. `SpeakItAppearanceSync` and `SpeakItSceneDelegate.sceneWillEnterForeground` write `overrideUserInterfaceStyle` themselves and nothing passes a scheme to SwiftUI. The first version of that write walked the whole tree — window, every controller, every view — which pinned every view to a style of its own; a pinned view ignores its window, so from the second switch onwards the window's write reached nobody by inheritance and the screen was repainted one view at a time in depth-first order, seen as a flicker around controls while Light or Dark was being applied. System was clean because `.unspecified` cleared the pins. The style is now written on the window only (`SpeakItAppearance.apply(to:)`), which everything under it inherits in one trait change; `ReleaseReadinessTests.testAppearanceIsWrittenOnlyOnTheWindow` holds that line. XCUITest cannot read the effective scheme, so the System check is still the gated probe `testSystemAppearanceHandsControlBackToIOS` (set `TEST_RUNNER_SPEAKIT_APPEARANCE_PROBE_DIR`, flip `xcrun simctl ui <udid> appearance` while it holds, and compare `simctl io screenshot` brightness; modes `system`, `control`, `explicit`).
