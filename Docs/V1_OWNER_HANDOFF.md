...
| Cost | A few hours of the Mac, unattended. Eight rows (baseline and seven PR rows) each run the corpus gate (about 9 min on the hosted runner in run 35810907291; `CLAUDE.md` says about 90 s on the author's Mac), the whole unit suite (about 9 min, `CLAUDE.md`) and the Release build (about 2.5 min, hosted): about 20 min a row, so about 2 h 45 min, before each row's clean simulator compile (derived data is per row and commit, so nothing is reused), its focused class, the baseline's NaturalLanguage readout and M2's model run. Schedule four hours; a faster local gate makes it shorter. `--only <label>` re-runs one row, and always runs the baseline beside it, so a single re-run costs two rows, about 45 min. No CI minutes, no paid inference. |
