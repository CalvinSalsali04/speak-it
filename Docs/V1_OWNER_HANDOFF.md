...
| | |
|---|---|
| Commits | The two manifest rows in the commit list below. `rc` is the candidate branch `claude/v1-reliability-nyngoe-rc` at its freeze, `3df9cb3`, which contains every V1 PR merged into it: #116, #117 and #119 to #156 (#150 arrived with #152; #121 is merged up to an earlier head, and its later commits change only this file, the manifest and the probe inputs, which the command reads from its own checkout). #118 is M2, done, and changes nothing in the candidate. It replaces the per-PR rows this file used to list; where each of their checks went is under the commit list. |
