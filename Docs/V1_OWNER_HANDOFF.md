...
two rows: `baseline` (main) and `rc` (the V1 candidate, carrying every check the
per-PR rows used to run). M2 has run and is done. M3 is the only Mac step
outside the command: captures typed into the Simulator with Apple Intelligence
on, using a list the command writes. M4, the fresh evaluation's app path, is
run by the scoring operator, not by hand. Nothing else on the Mac is run by hand.
...
| | |
|---|---|
| Commits | The two manifest rows in the commit list below. `rc` is the candidate branch `claude/v1-reliability-nyngoe-rc` at its freeze, `2def46e` (the merge of #158, whose last code change is `4fd8ae6`, onto `b5893e1`), which contains every V1 PR merged into it: #116, #117, #119 to #156 and #158 (#150 arrived with #152; #121 is merged up to an earlier head, and its later commits change only this file, the manifest and the probe inputs, which the command reads from its own checkout). #118 is M2, done, and changes nothing in the candidate. It replaces the per-PR rows this file used to list; where each of their checks went is under the commit list. |
...
the same baseline. #146 to #156, which never had rows, are covered the same way.
...
| | |
|---|---|
| Result | **Done, 2026-09-24: OUTCOME C** (neither candidate won: ranges 12/30, labels 16/30, the constant one-unit baseline 16/30). Run on the owner's Mac through Remote Control, recorded in `RESULT.md` on `claude/semantic-map-xklzll` at `eb13cd7`, and re-checked from the raw files by an independent grader. Production semantics are unchanged and #118 stays diagnostic-only. What follows is the plan as it was run. |
...
| Command | the manifest's former `semantic-units` row (removed after the run), which ran `./Tools/SemanticMap/UnitsExperiment/run.sh` in a clean worktree at `09a9eed` and writes into `semantic-units/units-experiment/` |
...
| Release blocker | No by itself. It chose no production change. |
| Status | done (`eb13cd7`, OUTCOME C) |
...
### M4. The fresh evaluation's app path (phase 13)

| | |
|---|---|
| Who | The phase 13 scoring operator, a session that wrote none of the candidate, through Remote Control. Nothing for the owner to type. It is listed because it needs the Mac. |
| Why a Mac | The `app` path saves every sealed capture through `createCaptureResult`, the repository call the capture screen uses, inside the unit-test host. The hosted simulator's tagger is blind, and `Tools/CI/whole-capture-app-path.sh` refuses a blind export (exit 4): it would describe the degraded policy, not a phone. |
| Commit | the `rc` pin, `2def46e` |
| Command | from a clean worktree at the pin outside the owner's checkout: `Tools/CI/whole-capture-app-path.sh <sealed set> <output outside the repository>`, then `python3 Tools/CorpusRunner/wholecapture/score.py score` once, with `--seal`, as `v1/phase13/PROTOCOL.md` section 5 says. Use the repository's scorer, not the copy beside the sealed set, which cannot run where it is. |
| Expected evidence | Exit 0; a line per sealed id; the receipt; per-family whole-capture rates. Only rates leave the operator's session, never capture text. |
| Release blocker | Yes: phase 13 is the release's generalization measure. |
| Status | queued, after M1 says the pin compiles on a sighted simulator |

## Merge order and conflicts (for whoever merges)

**Resolved.** Every line below was resolved when its PRs were merged into the
candidate, which is now frozen; the list is kept as the record of how. The only
merge left is the candidate into main, which carries these resolutions with it.
