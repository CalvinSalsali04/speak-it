...
- A sentence naming a *named* place and a time ("When I go to Sobeys, remind me to get cheese, eggs, and bread in one hour") keeps the stated time as the one real trigger — a named business cannot be geofenced, so the hour is the only enforceable half — and still uses the place to name the list. The delay may also sit inside the place clause ("When I go to Sobeys in an hour…") with the same reading. A sentence combining a *saved* place with a time ("Remind me to feed the dog when I get home tonight") is different: Speak It could enforce either half, both single-half readings are wrong in a way the person would feel, so the capture is held in Needs review and neither trigger fires until they choose. Place-only sentences keep their place trigger unchanged.
...
- Because that placement is derived from the device clock, the two UI tests that open the List from a card load `--load-shopping-examples` rather than `--load-today-examples`. That fixture pins its own reference frame (`SampleDataLibrary.Shopping`) the way `Tools/PipelineProbe` does, so its list is overdue — and therefore in the always-open Now section — at every hour. Read at the device clock, "buy milk after work" resolves to 5 PM and rolls to tomorrow after it, which moved the card into the collapsed Coming up disclosure, where the row is still findable but takes no taps; the tests then failed every evening on unchanged code.
