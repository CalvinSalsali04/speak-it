...
final class SwiftDataThoughtRepositoryTests: XCTestCase {
    ...
    // MARK: - Launch asks about a stored place-and-time row nobody was asked about

    /// The DEL-11 shape: "when I get home tomorrow" stored by a build before
    /// the capture-time hold, with Home, a date-only day, a 9 AM clock and no
    /// review flag.
    ///
    /// Dates are built in the machine's zone, never under a fixture pin,
    /// because the tests read `ReminderScheduleRequest`, which compares
    /// against the real clock. The title polish is stamped as already run so
    /// a changed title can only come from the pass under test.
    private func storedPlaceAndTimeRow(
        isReviewed: Bool = false,
        timeSetByHand: Bool = false,
        needsClarification: Bool = false
    ) throws -> CapturedItem {
        UserDefaults.standard.set(
            SwiftDataThoughtRepository.titlePolishVersion,
            forKey: SwiftDataThoughtRepository.titlePolishVersionKey
        )
        ...
        let row = CapturedItem(
            ...
            temporalIntent: TemporalIntent(kind: .dateOnly, isUserEdited: timeSetByHand),
    ...
    private static let storedModificationDate = Date(timeIntervalSince1970: 1_700_000_000)

    /// A stored place-and-time row that no one was asked about goes to review
    /// at launch, carrying its words, its place and its clock unchanged, and
    /// one review gives the clock back.
    ///
    /// Before this pass the row sat in Today showing its date while the
    /// scheduler refused its clock and no region watched its place: nothing
    /// fired and nothing asked (REHEARSAL F7).
    ///
    /// Falsifier: after launch the row is still out of review, or it lost a
    /// word, its place, its day or its clock, or reviewing it does not arm
    /// the clock it was stored with.
    func testLaunchMovesAStoredPlaceAndTimeRowNobodyWasAskedAboutIntoReview() throws {
        let row = try storedPlaceAndTimeRow()
        let fire = try XCTUnwrap(row.reminderDate)
        let due = row.dueDate
        let place = row.locationIntent
        let time = row.temporalIntent
        XCTAssertNil(ReminderScheduleRequest(item: row), "the silent state: the clock is refused")
        XCTAssertTrue(row.belongsInToday, "the silent state: it shows in Today, not in review")

        repository.recoverUnorganizedCaptures()

        XCTAssertTrue(row.needsClarification, "the row is in review")
        XCTAssertEqual(row.clarificationRequirement, .combinedTimeAndPlace)
        XCTAssertFalse(row.belongsInToday, "a row in review is not also in Today")
        XCTAssertFalse(row.isReviewed, "the pass does not answer the question for the person")
        XCTAssertNotEqual(row.lastModifiedAt, Self.storedModificationDate, "the change is dated for sync")
        XCTAssertEqual(row.originalTextSegment, "Remind me to call Mom when I get home tomorrow")
        XCTAssertEqual(row.displayTitle, "Call Mom")
        XCTAssertEqual(row.locationIntent, place, "the place is kept")
        XCTAssertEqual(row.temporalIntent, time, "the time is kept")
        XCTAssertEqual(row.reminderDate, fire, "the clock is kept")
        XCTAssertEqual(row.dueDate, due, "the day is kept")
        XCTAssertNil(ReminderScheduleRequest(item: row), "still refused until the person answers")

        try repository.markReviewed(row)

        XCTAssertFalse(row.needsClarification)
        XCTAssertEqual(
            ReminderScheduleRequest(item: row)?.fireDate,
            fire,
            "one review arms the clock the row was stored with"
        )
    }

    /// The pass reads only the rows the scheduler refuses and nothing asks
    /// about. A reviewed row, a row whose time the person set, a row already
    /// in review, a done or archived row, and a place or a time alone are
    /// each left exactly as stored.
    ///
    /// Falsifier: any of them gains the review flag or a new modification
    /// date, or a reviewed or hand-set row stops arming its clock.
    func testLaunchLeavesAReviewedOrHandSetOrAlreadyHeldPlaceAndTimeRowAlone() throws {
        let reviewed = try storedPlaceAndTimeRow(isReviewed: true)
        let timeSetByHand = try storedPlaceAndTimeRow(timeSetByHand: true)
        let alreadyInReview = try storedPlaceAndTimeRow(needsClarification: true)
        let done = try storedPlaceAndTimeRow()
        done.completedAt = Self.storedModificationDate
        let archived = try storedPlaceAndTimeRow()
        archived.isArchived = true
        let placeAlone = CapturedItem(
            originalTextSegment: "Remind me to take out the garbage when I get home",
        ...
        timeAlone.locationIntent = nil
        container.mainContext.insert(placeAlone)
        try container.mainContext.save()
        XCTAssertFalse(placeAlone.constrainsBothPlaceAndTime)
        XCTAssertFalse(timeAlone.constrainsBothPlaceAndTime)

        repository.recoverUnorganizedCaptures()

        for (name, row, wasInReview) in [
            ("reviewed", reviewed, false),
            ("time set by hand", timeSetByHand, false),
            ("already in review", alreadyInReview, true),
            ("done", done, false),
            ("archived", archived, false),
            ("place alone", placeAlone, false),
            ("time alone", timeAlone, false),
        ] {
            XCTAssertEqual(row.needsClarification, wasInReview, "\(name): review flag changed")
            XCTAssertEqual(row.lastModifiedAt, Self.storedModificationDate, "\(name): row was touched")
            XCTAssertFalse(
                SwiftDataThoughtRepository.isSelectedByUnaskedPlaceAndTimePass(row),
                "\(name): selected by the pass"
            )
        }
        XCTAssertNotNil(ReminderScheduleRequest(item: reviewed), "a reviewed row still arms")
        XCTAssertNotNil(ReminderScheduleRequest(item: timeSetByHand), "a hand-set time still arms")
    }

    /// The pass is idempotent. A row it moved is in review, so the next
    /// launch does not select it, and nothing in the store changes.
    ///
    /// Falsifier: the second launch re-dates the row, changes any field, or
    /// leaves unsaved changes in the context.
    func testASecondLaunchChangesNothing() throws {
        let row = try storedPlaceAndTimeRow()

        repository.recoverUnorganizedCaptures()
        XCTAssertTrue(row.needsClarification)
        let movedAt = row.lastModifiedAt
        let fire = row.reminderDate
        let place = row.locationIntent
        let time = row.temporalIntent
        XCTAssertFalse(SwiftDataThoughtRepository.isSelectedByUnaskedPlaceAndTimePass(row))

        repository.recoverUnorganizedCaptures()

        XCTAssertTrue(row.needsClarification)
        XCTAssertFalse(row.isReviewed)
        XCTAssertEqual(row.lastModifiedAt, movedAt, "the second launch re-dated the row")
        XCTAssertEqual(row.reminderDate, fire)
        XCTAssertEqual(row.locationIntent, place)
        XCTAssertEqual(row.temporalIntent, time)
        XCTAssertFalse(container.mainContext.hasChanges, "the second launch left changes behind")
    }
    ...
    func testRetryingAfterASplitLeavesNothingOfTheAttempt() async throws {
        ...
        XCTAssertFalse(replacement.settled.retrySourceSurvived)
        XCTAssertEqual(
            replacement.deletedRows,
            shown.count + 1,
            "the retry must delete the rows the attempt has now, the split-off row included"
        )
    ...
    func testRetryingAfterAMergeDeletesEverythingWithoutTouchingTheMergedAwayRow() async throws {
        ...
        XCTAssertFalse(
            replacement.settled.retrySourceSurvived,
            "a clean replacement was reported as a failure, because a row Merge had already deleted was deleted again"
        )
        XCTAssertEqual(replacement.deletedRows, 1, "only the merged row was left to delete")
        try assertAttemptIsGone(attemptID, retry: retry)
    }
    ...
    func testRetryingAfterUndoDeletesEverythingAndASecondDeletionIsANoOp() async throws {
        ...
        XCTAssertFalse(replacement.settled.retrySourceSurvived)
        XCTAssertEqual(replacement.deletedRows, 1, "only the row Undo kept was left to delete")
        try assertAttemptIsGone(attemptID, retry: retry)
        XCTAssertEqual(
    ...
    func testRetryingAfterOrganizeAgainLeavesNothingOfTheAttempt() async throws {
        ...
        let rowsAtRetry = attempt.session.items.count
        ...
        XCTAssertEqual(
            replacement.deletedRows,
            rowsAtRetry,
            "the retry must delete the rows Organize again left, not the ones the screen was shown"
        )
        try assertAttemptIsGone(attemptID, retry: retry)
    }

    /// "Try saying it again" starts the new recording about a quarter of a
    /// second after the tap, on a flow whose premise is that the person is
    /// about to repeat themselves. Saying the same words again inside
    /// `inAppCaptureDeduplicationWindow` makes the store hand back the
    /// attempt's own session instead of storing a second one, and retiring
    /// the attempt then would delete the capture the store just returned:
    /// both attempts gone, and the screen told the save succeeded.
    ///
    /// The real echo is refused twice over (it is not new, and it is the
    /// attempt), so this test first checks the real echo, then gives the same
    /// session back labelled as new, so the session-identity check has to
    /// hold by itself. No shipped path returns an existing session as new
    /// today; this is the guard for the one that does (#128's reused session
    /// id is the nearest).
    ///
    /// Falsifier: drop `attempt != result.session.id` from `replaces`. The
    /// relabelled echo then replaces the attempt, `retire` deletes it, and
    /// every assertion after the relabel fails.
    func testARetryHandedBackTheAttemptsOwnSessionDoesNotDeleteIt() async throws {
        let saidAt = Date.now
        let attempt = try await repository.createCaptureResult(
            text: "Something about the bank and the fee thing",
            source: .inAppVoice,
        ...
        let attemptID = attempt.session.id
        let attemptRows = attempt.session.items.count
        let echo = try await repository.createCaptureResult(
            text: "something about the bank and the fee thing",
            source: .inAppVoice,
            createdAt: saidAt.addingTimeInterval(5),
            schedulesReminders: false
        )
        XCTAssertEqual(echo.session.id, attemptID, "precondition: the store handed back the attempt's own session")
        XCTAssertFalse(echo.createdNewCapture, "precondition: the store stored nothing new")
        XCTAssertFalse(
            CaptureRetryReplacement.replaces(attempt: attemptID, with: echo),
            "the store's own echo of the attempt must not replace it"
        )

        let relabelled = CaptureCreationResult(session: echo.session, items: echo.items, createdNewCapture: true)
        let save = try saveRetry(of: attemptID, with: relabelled)

        XCTAssertFalse(save.replaces, "a result carrying the attempt's own session must never replace the attempt")
        XCTAssertNil(save.deletedRows, "retire ran against the attempt's own session")
        XCTAssertFalse(save.settled.retrySourceSurvived)
        try assertAttemptSurvives(attemptID, rows: attemptRows)
    }

    /// A retry that stores nothing new replaces nothing. Here the person
    /// repeats, inside the deduplication window, a thought they captured just
    /// before the unclear attempt, so the store hands back that earlier
    /// capture: a different session from the attempt, marked as not new.
    /// Retiring the attempt would delete it while the retry stored nothing in
    /// its place, and the attempt's words would be gone.
    ///
    /// Falsifier: drop `result.createdNewCapture &&` from `replaces`. The
    /// earlier capture's session is not the attempt's, so `replaces` turns
    /// true, `retire` deletes the attempt, and the survival assertions fail.
    func testARetryThatStoredNothingNewDoesNotDeleteTheAttempt() async throws {
        let saidAt = Date.now
        let earlier = try await repository.createCaptureResult(
        ...
        let attempt = try await repository.createCaptureResult(
            text: "Something about the bank and the fee thing",
            source: .inAppVoice,
            createdAt: saidAt.addingTimeInterval(2),
            schedulesReminders: false
        )
        let attemptID = attempt.session.id
        let attemptRows = attempt.session.items.count
        let retry = try await repository.createCaptureResult(
            text: "Call the bank about the overdraft fee",
            source: .inAppVoice,
            createdAt: saidAt.addingTimeInterval(6),
            schedulesReminders: false
        )
        XCTAssertFalse(retry.createdNewCapture, "precondition: the store handed back the earlier capture")
        XCTAssertEqual(retry.session.id, earlier.session.id, "precondition: the capture handed back is the earlier one")
        XCTAssertNotEqual(retry.session.id, attemptID, "precondition: the capture handed back is not the attempt")

        let save = try saveRetry(of: attemptID, with: retry)

        XCTAssertFalse(save.replaces, "a retry that stored nothing new must not replace the attempt")
        XCTAssertNil(save.deletedRows, "retire ran for a retry that stored nothing")
        XCTAssertFalse(save.settled.retrySourceSurvived)
        try assertAttemptSurvives(attemptID, rows: attemptRows)
    }
    ...
    func testARetryWhoseDeletionIsRefusedKeepsTheAttemptAndSaysSo() async throws {
        ...
        let replacement = try replaceAttempt(attemptID, with: retry, in: refusing)

        XCTAssertTrue(
            replacement.settled.retrySourceSurvived,
            "a deletion the store refused was reported as done, and the person was not told the attempt is still there"
        )
        XCTAssertNil(replacement.deletedRows, "a refused deletion must throw, not report rows as gone")
    ...
    private struct RetrySave {
        let replaces: Bool
        let settled: CaptureSaveSettlement.Settled
        let deletedRows: Int?
    }
    ...
    private func replaceAttempt(
        ...
        let save = try saveRetry(of: attemptID, with: retry, in: store)
        XCTAssertTrue(save.replaces, "precondition: the retry is a new capture, not the attempt handed back")
        return save
    }

    /// `CaptureView.save`'s retry settlement as the view runs it, whether or
    /// not the result replaces the attempt.
    private func saveRetry(
        of attemptID: UUID,
        with retry: CaptureCreationResult,
        in store: (any ThoughtRepository)? = nil
    ) throws -> RetrySave {
        ...
        let settled = CaptureSaveSettlement.settle(
            ...
            recordSaved: {},
            deleteRetrySource: {
                deletedRows = try CaptureRetryReplacement.retire(attempt: attemptID, in: target)
            },
            publishes: { true },
            clearDraftOfEndedScreen: {}
        )
        return RetrySave(replaces: replaces, settled: settled, deletedRows: deletedRows)
    }

    /// The attempt is whole: its session, every row it had, and no iCloud
    /// tombstone that would delete it on another device.
    private func assertAttemptSurvives(
        _ attemptID: UUID,
        rows: Int,
        ...
        XCTAssertTrue(
            try durable.fetch(FetchDescriptor<CaptureSession>()).contains { $0.id == attemptID },
            "the retry deleted the attempt, and its original words, while storing nothing to replace it",
            file: file,
            line: line
        )
        XCTAssertEqual(
            try durable.fetch(FetchDescriptor<CapturedItem>())
                .filter { $0.captureSession?.id == attemptID }.count,
            rows,
            "the attempt lost rows to a retry that did not replace it",
            file: file,
            line: line
        )
        XCTAssertFalse(
            ICloudDeletionStore.records().contains { $0.id == attemptID },
            "a retry that did not replace the attempt left an iCloud tombstone for it",
            file: file,
            line: line
        )
    }
    ...
    /// Launch leaves a named place held beside a time set by hand as it is:
    /// the hold stays (DEL-18: the launch pass that released named-place holds,
    /// `resolveCombinedPlaceAndTimeHoldouts`, is gone, since capture now holds
    /// them on purpose), and the time the person set is not re-read.
    ///
    ...
    /// dropped. The dates would survive such a pass if it only filled empty
    /// ones, which is why the intent is asserted on.
    func testLaunchLeavesAHoldoutAndItsTimeSetByHandAlone() throws {
        let createdAt = Date.now.addingTimeInterval(-120)
        let chosen = try handSetMoment()
        ...
        let heldPlace = stuck.locationIntent

        repository.recoverUnorganizedCaptures()

        XCTAssertEqual(stuck.locationIntent, heldPlace, "launch released a named place held beside a time")
